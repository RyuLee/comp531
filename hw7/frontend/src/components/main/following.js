...
const Follower = ({name, avatar, headline, dispatch}) => (
    ...
    <span className="follower-headline">{headline}</span><br></br><button className="btn-flat waves-effect waves-light" 
    ...
    render() { return (
        <div>
            ...
            <li>
                ...
                <input id="inputFollow"style={{position: 'relative',left: '20px',width: '40%'}} type="text" placeholder="Add Firends"
                ref={(node) => this.newFollower = node }
                        onChange={(e) => {
                            this.forceUpdate()
                        }}/>
                    { !(this.newFollower && this.newFollower.value && this.newFollower.value.length > 0) ? '' :
                    <input id="btn-follow" className="btn-flat waves-effect waves-light" type="button"
                        onClick={() => {
