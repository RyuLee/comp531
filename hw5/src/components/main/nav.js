...
const Nav2 = ({username, onProfile, dispatch}) => (
    ...
    <Navbar brand='RiceBook' right style={{position:'relative'}}>
      ...
      {username.length == 0?'':
      <div>
      { onProfile ? <NavItem  href="#" onClick={() => { dispatch(navToMain()) }}>Home</NavItem>
      :<NavItem  href="#" onClick={() => { dispatch(navToProfile()) }}>Edit Your Profile</NavItem>
      }
      <NavItem  href="#" onClick={() => { dispatch(logout()) }}>Log out {username} </NavItem>
    </div>}
