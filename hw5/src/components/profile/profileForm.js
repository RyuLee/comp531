            ...
            <form onSubmit={(e) => {
                ...
                <div className="row">
                    <div className="input-field col s6">
                        Email Address:
                        <input type="email" name="email" placeholder={this.props.oldEmail}  ref={(node)=>this.email = node}></input>
                    </div>
                     <div className="input-field col s6">
                        Zip Code(Five digits):
                        <input type="number" name="zip" pattern="^\d{5}$" placeholder={this.props.oldZipcode}  ref={(node)=>this.zipcode = node}></input>
                    </div>
                </div>
                <div className="row">
                    <div className="input-field col s6">
                        Password:(The length should be from 5 to 12.)
                        <input type="password" name="password" pattern="^\w{5,12}$"  ref={(node)=>this.password = node}></input>
                    </div>
                    <div className="input-field col s6">
                        Password Confirmation:
                        <input type="password" name="confirmation" pattern="^\w{5,12}$"  ref={(node)=>this.pwconf = node}></input>
                    </div>
                </div>
                <div className="row">
                    <div>
